...
public extension Model {
    func events<Case>(ofCase casePath: CasePath<Event, Case>) -> AsyncStream<Case> where Event: Sendable {
        AsyncStream(events().compactMap { event in
            casePath.extract(from: event)
        })
    }

    func events<M: Model, Case>(ofCase casePath: CasePath<M.Event, Case>, fromType modelType: M.Type = M.self) -> AsyncStream<(case: Case, model: M)> where M.Event: Sendable {
        AsyncStream(events(fromType: modelType).compactMap { event, model in
